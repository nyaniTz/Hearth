...
if (isset($_GET['user']) && count($_GET) > 1) {
    ...
    $result = updatePatient($user, $field_list, $value_list);

    // Echo a message based on the result
    if ($result > 0) {
        echo "Patient details updated successfully.". " DEVICE ID IS:  ";//.$_GET['latitude'];
    } else {
        echo "Failed to update patient details.";
    }
}
