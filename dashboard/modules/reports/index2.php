					...
					<?php
						...
						if ($result->num_rows > 0) {
						    while($row = $result->fetch_assoc()) {
						        echo "<option value='".$row["Tables_in_health"]."'>".$row["Tables_in_health"]."</option>";
						    }
						} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    echo '<script>
            async function makePrediction() {
                ...
                document.getElementById("result").innerText = `AI Predicted State: ${predictedState}, Probability: ${predictedValue.toFixed(2)} % Sure`;
